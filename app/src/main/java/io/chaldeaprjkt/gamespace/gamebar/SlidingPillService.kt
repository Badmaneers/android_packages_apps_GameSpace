...
class SlidingPillService @Inject constructor(
    ...
    private fun showPill(pill: SlidingPillNotification) {
        ...
        when {
            ...
            entryDir == "top" -> {
                state.startX = hMargin.toFloat()
                state.targetX = state.startX
                state.currentY = -pillHeight.toFloat()
            }
            entryDir == "bottom" -> {
                state.startX = hMargin.toFloat()
                state.targetX = state.startX
                state.currentY = displayHeight.toFloat()
            }
            else -> {
                state.startX = displayWidth.toFloat()
                state.targetX = if (slideAcross) -state.width.toFloat()
    ...
    private fun getMaxLanes(): Int = appSettings.slidingPillLaneCount.coerceIn(1, 8)
    ...
    private fun getPillHeight(): Int {
        val fontSize = appSettings.slidingPillFontSize.toFloat()
        val lineHeight = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_SP, fontSize, context.resources.displayMetrics
        )
        val lines = if (appSettings.slidingPillSize == "compact") 1 else 2
    ...
    private fun getNotificationAreaTop(): Int {
        ...
        return when (appSettings.slidingPillPosition) {
            "top" -> sbHeight + 4.dp
            "upper_third" -> (displayHeight / 3)
            "middle" -> (displayHeight / 2) - getPillHeight() / 2
            "lower_third" -> (displayHeight * 2 / 3)
            "bottom" -> displayHeight - getPillHeight() - 8.dp
            else -> sbHeight + appSettings.slidingPillCustomOffset.dp
        }
    }
    ...
    private fun createPillView(): FrameLayout {
        ...
        val textView = TextView(context).apply {
            id = TEXT_VIEW_ID
            val initialMaxLines = if (appSettings.slidingPillSize == "compact") 1 else 2
    ...
    private fun bindPillView(view: View, pill: SlidingPillNotification, state: SlidingPillState) {
        ...
        val showIcon = appSettings.slidingPillShowIcon
        ...
        textView.layoutParams = FrameLayout.LayoutParams(
            ...
            val iconW = if (showIcon && pill.appIcon != null) iconSize + contentPadding * 2 else 0
            leftMargin = iconW + 6.dp
        }

        val messageWidth = measureTextWidth(displayText, fontSize)
        val iconW = if (showIcon && pill.appIcon != null) iconSize + contentPadding * 2 else 0
        ...
        val baseMaxLines = if (appSettings.slidingPillSize == "compact") 1 else 2
        when (appSettings.slidingPillOverflowMode) {
            "ellipsis" -> {
    ...
    private fun updatePillAppearance(frame: FrameLayout, pill: SlidingPillNotification, pillHeight: Int) {
        ...
        val cornerRadius = if (appSettings.slidingPillCapsuleMode) {
            (pillHeight / 2f)
        } else {
            appSettings.slidingPillCornerRadius.dp.toFloat()
        }
        ...
        frame.elevation = (appSettings.slidingPillShadowStrength / 100f * 16.dp).toFloat()
    }
            ...
            when (s.phase) {
                PillPhase.ENTERING -> {
                    ...
                    if (s.enterProgress >= 1f) {
                        s.enterProgress = 1f
                        if (appSettings.slidingPillSlideAcross) {
